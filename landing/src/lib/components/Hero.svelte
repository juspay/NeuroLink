<script lang="ts">
  ...
  onMount(() => {
    ...
    (async () => {
      ...
      tl.from(q("[data-hero-badge]"), { y: 30, opacity: 0, duration: 0.6 }, 0.2)
        .from(
...
<section
  bind:this={heroEl}
  class="relative min-h-screen pt-24 pb-16 px-6 flex items-center max-w-[1400px] mx-auto overflow-hidden"
>
  ...
  >
    ...
    <div class="lg:col-span-3 space-y-8">
      ...
      <p
        data-hero-subtitle
        class="text-lg text-ds-text-tertiary max-w-xl leading-relaxed"
      >
      ...
      <div data-hero-cta>
        <a
          ...
          class="group inline-flex items-center rounded-full border border-ds-border bg-ds-surface-3 hover:border-ds-border-hover hover:bg-ds-surface-4 transition-all duration-200"
        >
          <span class="px-5 py-3 text-sm font-medium text-ds-text-primary">
            Get Started
          </span>
          <span class="w-px h-8 bg-ds-border"></span>
          <span
            class="px-5 py-3 text-sm font-mono text-ds-text-tertiary group-hover:text-ds-text-secondary transition-colors"
          >
