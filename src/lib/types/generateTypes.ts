...
export type GenerateOptions = {
   ...
   *
   * Enable transcription of audio files provided in input.files.
   * Audio files are auto-detected by extension (.mp3, .wav, .ogg, etc.).
   * The transcription result will be returned in the result's `transcription` field.
   *
   * @example Basic STT with auto-detection
   * ```typescript
   * const result = await neurolink.generate({
   *   input: { text: "Transcribe this audio", files: ["audio.mp3"] },
   *   provider: "google-ai"
   * });
   * console.log(result.transcription?.text); // Transcribed text
   * ```
   *
   * @example Explicit STT options
   * ```typescript
   * const result = await neurolink.generate({
   *   input: { text: "Transcribe", files: [audioBuffer] },
   *   provider: "google-ai",
   *   stt: { languageCode: "en-US", enableAutomaticPunctuation: true }
   * });
   * ```
   */
  stt?: STTOptions;
...
export type GenerateResult = {
   ...
   *
   * Contains the transcribed text and metadata when STT is enabled and audio files are provided in input.files.
   * Generated by STTProcessor.transcribeAudio() using the specified provider.
   *
   * @example Accessing STT transcription
   * ```typescript
   * const result = await neurolink.generate({
   *   input: { text: "Transcribe this", files: ["audio.mp3"] },
   *   provider: "google-ai"
   * });
   *
   ...
   *     console.log(`Detected language: ${result.transcription.language}`);
   *   }
   * }
   * ```
   */
...
export type TextGenerationOptions = {
  ...
  input?: {
    ...
    pdfFiles?: Array<
      Buffer | string | import("./fileTypes.js").FileWithMetadata
    >; // Support for PDF inputs (for image generation with Vertex AI)
   ...
   * The transcription result will be returned in the result's `transcription` field.
   *
   * This is only used when STT is explicitly enabled in GenerateOptions and passed through
   * to TextGenerationOptions during the conversion process.
   */
  stt?: STTOptions;
