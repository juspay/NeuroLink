...
export function ensureNestedSchemaTypes(
  ...
  if (!result.type) {
    ...
    // If it has enum, infer from enum values
    else if (
      result.enum &&
      Array.isArray(result.enum) &&
      result.enum.length > 0
    ) {
      const firstValue = result.enum[0];
 ...
 * Uses OpenAPI 3.0 target to produce nullable: true instead of anyOf with null,
 * which is required for Google Vertex AI compatibility.
 */
