...
export class ConversationMemoryManager {
  ...
  async storeConversationTurn(
    ...
    try {
      ...
      // Enforce per-session turn limit (each turn = 2 messages: user + assistant)
      const maxMessages = (this.config.maxTurnsPerSession || 50) * 2;
      ...
      logger.debug("Conversation turn stored", {
        ...
        turnCount: session.messages.length / 2, // Each turn = 2 messages
   ...
   * ULTRA-OPTIMIZED: Calculate turns from message count (each turn = 2 messages)
   */
  async getStats(): Promise<ConversationMemoryStats> {
    ...
    const totalTurns = sessions.reduce(
      (sum, session) => sum + session.messages.length / 2,
      0,
    );
