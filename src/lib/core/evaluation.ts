...
`;
    ...
    try {
      ...
      // Fallback with clamped values
      const validatedResult = {
        ...parsed,
        relevance: Math.max(1, Math.min(10, parsed.relevance || 1)),
        accuracy: Math.max(1, Math.min(10, parsed.accuracy || 1)),
        completeness: Math.max(1, Math.min(10, parsed.completeness || 1)),
        overall: Math.max(1, Math.min(10, parsed.overall || 1)),
        evaluationModel: `${evaluationProvider}/${evaluationModel}`,
        evaluationTime: Date.now() - startTime,
        evaluationProvider,
        evaluationAttempt: 1,
