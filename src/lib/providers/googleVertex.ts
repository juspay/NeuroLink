...
export class GoogleVertexProvider extends BaseProvider {
  ...
  /**
   * Create Google GenAI client configured for Vertex AI endpoint
   *
   * The @google/genai library supports Vertex AI through the vertexai configuration.
   * We need to pass project and location to tell it to use Vertex AI endpoints instead of AI Studio.
   */
  private async createVertexGenAIClient(): Promise<GenAIClient> {
    ...
    // Ensure Vertex settings are configured (sets up GOOGLE_APPLICATION_CREDENTIALS)
    await createVertexSettings(this.location);

    // Create client configured for Vertex AI
    // The vertexai configuration tells the library to use Vertex AI endpoints
    // Note: TypeScript types don't expose vertexai config, but it works at runtime
