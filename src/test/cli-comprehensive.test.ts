...
async function execCLI(
	...
	return new Promise((resolve) => {
		...
		const sanitizedArgs = args.map((arg) =>
			typeof arg === "string" ? arg.replace(/\0/g, "") : arg,
		);
...
describe("Comprehensive CLI Tests", () => {
	...
	function createTestFixtures() {
		...
		// Large prompts file
		writeFileSync(
			join(FIXTURES_DIR, "large-prompts.txt"),
		...
		writeFileSync(
			join(FIXTURES_DIR, "special-chars.txt"),
			[
				"Prompt with emojis 🚀🤖🎯",
				"Symbols @#$%^&*()[]{}|\\:\";'<>?,./`~",
		...
		writeFileSync(
			join(FIXTURES_DIR, "invalid-config.json"),
			"{ invalid json }",
		);
	}
	...
	describe("Command Line Argument Parsing", () => {
		...
		it("should handle quoted arguments with spaces", async () => {
			const result = await execCLI([
				"generate",
				"This is a test prompt with spaces",
			]);
	...
	describe("Provider Commands", () => {
		it("should handle provider status command", async () => {
			...
			expect(result.stdout + result.stderr).toMatch(
				/(provider|status|openai|bedrock|vertex)/i,
			);
		});
		...
		it("should handle provider list command", async () => {
			...
			expect(result.stdout + result.stderr).toMatch(
				/(provider|available|openai|bedrock|vertex)/i,
			);
		});
	...
	describe("Text Generation Commands", () => {
		it("should handle basic text generation", async () => {
			...
			expect(result.stdout + result.stderr).toMatch(
				/(generate|error|api|key|provider)/i,
			);
		});
		...
		it("should handle generation with provider specification", async () => {
			...
			for (const provider of providers) {
				...
				expect(result.stdout + result.stderr).toMatch(
					new RegExp(`(${provider}|error|api|key)`, "i"),
				);
			}
		});
	...
	describe("Streaming Commands", () => {
		it("should handle basic streaming command", async () => {
			...
			expect(result.stdout + result.stderr).toMatch(
				/(stream|error|api|key|provider)/i,
			);
		});
	...
	describe("Batch Processing Commands", () => {
		it("should handle valid batch file", async () => {
			const result = await execCLI(
				["batch", join(FIXTURES_DIR, "valid-prompts.txt")],
				{ timeout: 90000 },
			);
			expect(result.stdout + result.stderr).toMatch(
				/(batch|process|error|api|key)/i,
			);
		}, 90000);
		...
		it("should handle nonexistent batch file", async () => {
			const result = await execCLI([
				"batch",
				join(FIXTURES_DIR, "nonexistent.txt"),
			]);
			expect(result.exitCode).not.toBe(0);
		...
		it("should handle large batch file", async () => {
			const result = await execCLI(
				["batch", join(FIXTURES_DIR, "large-prompts.txt")],
				{
		...
		it("should handle batch file with special characters", async () => {
			const result = await execCLI(
				["batch", join(FIXTURES_DIR, "special-chars.txt")],
				{ timeout: 90000 },
			);
			expect(result.stdout + result.stderr).toMatch(
				/(batch|process|error|api|key)/i,
		...
		it("should handle batch with concurrent processing", async () => {
			const result = await execCLI(
				["batch", join(FIXTURES_DIR, "valid-prompts.txt"), "--concurrent", "3"],
				{ timeout: 90000 },
			);
	...
	describe("Error Handling and Recovery", () => {
		...
		it("should handle rate limiting gracefully", async () => {
			...
			const promises = Array(5)
				.fill(0)
				.map(() => execCLI(["generate", "quick test"]));
		...
		it("should handle invalid API responses", async () => {
			...
			expect(result.stderr).toMatch(
				/(api.*key|credential|authentication|network|connection|endpoint)/i,
			);
		});
		...
		it("should handle SIGINT gracefully", async () => {
			const child = spawn(
				"node",
				[CLI_PATH, "generate", "Long running prompt"],
				{
					stdio: ["pipe", "pipe", "pipe"],
				},
			);
		...
		it(
			...
			async () => {
				...
				const promises = Array(3)
					.fill(0)
					.map(() => execCLI(["batch", testFile], { timeout: 90000 }));
	...
	describe("Platform-Specific Behavior", () => {
		...
		it(
			"should handle environment-specific newlines",
			async () => {
	...
	describe("Performance and Scalability", () => {
		...
		it("should handle large output streams", async () => {
			...
			writeFileSync(
				largePromptFile,
				Array(50).fill("Generate a long detailed response").join("\n"),
			);
		...
		it("should handle memory-intensive operations", async () => {
			...
			const promises = Array(20)
				.fill(0)
				.map((_, i) => execCLI(["generate", `test ${i}`]));
	...
	describe("Integration Testing", () => {
		it("should work with package managers", async () => {
			...
			try {
				const npmResult = execSync("npm list @juspay/neurolink", {
					encoding: "utf8",
				});
	...
	describe("Regression Testing", () => {
		...
		it("should handle legacy configuration formats", async () => {
			...
			expect(result.stdout + result.stderr).toMatch(
				/(config|import|legacy|migration)/i,
			);
		});
	...
	describe("Edge Cases and Corner Cases", () => {
		...
		it("should handle interrupted operations gracefully", async () => {
			const child = spawn(
				"node",
				[CLI_PATH, "batch", join(FIXTURES_DIR, "large-prompts.txt")],
				{
					stdio: ["pipe", "pipe", "pipe"],
				},
			);
